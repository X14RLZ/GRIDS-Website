...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // 1. Initialize User Database if empty
    const storedUsers = localStorage.getItem('grids_users');
    if (!storedUsers) {
      const initialAdmin = {
        email: 'admin@baguio.gov.ph',
      ...
      localStorage.setItem('grids_users', JSON.stringify([initialAdmin]));
    }
  ...
  const handleUpdateUser = (updatedUser: User) => {
    ...
    // Update the master database as well
    const storedUsers = JSON.parse(localStorage.getItem('grids_users') || '[]');
    const updatedUsers = storedUsers.map((u: any) => u.email === updatedUser.email ? { ...u, ...updatedUser } : u);
